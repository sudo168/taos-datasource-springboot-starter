...
import net.ewant.taos.exception.TaosExhaustedPoolException;
...
public abstract class TaosPool<T> implements Closeable {
    protected GenericObjectPool<T> internalPool;

    public TaosPool(final GenericObjectPoolConfig poolConfig, PooledObjectFactory<T> factory) {
    ...
    public T getResource() {
        try {
            ...
            if (null == nse.getCause()) { // The exception was caused by an exhausted pool
                throw new TaosExhaustedPoolException("Could not get a resource since the pool is exhausted", nse);
            }
            // Otherwise, the exception was caused by the implemented activateObject() or ValidateObject()
            throw new TaosException("Could not get a resource from the pool", nse);
        } catch (Exception e) {
            throw new TaosConnectionException("Could not get a resource from the pool", e);
        }
    }
    ...
    protected void returnResourceObject(final T resource) {
        ...
        try {
            ...
            throw new TaosException("Could not return the resource to the pool", e);
        }
    }
    ...
    protected void returnBrokenResourceObject(final T resource) {
        try {
            ...
            throw new TaosException("Could not return the broken resource to the pool", e);
        }
    }
    ...
    protected void closeInternalPool() {
        try {
            ...
            throw new TaosException("Could not destroy the pool", e);
        }
    }
    ...
    public void addObjects(int count) {
        try {
            ...
            throw new TaosException("Error trying to add idle objects", e);
        }
    }
}
